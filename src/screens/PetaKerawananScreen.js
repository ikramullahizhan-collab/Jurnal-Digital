    ...
    try {
      ...
      if (response) {
        ...
        const processedData = Array.isArray(rawData) ? rawData.map(siswa => {
          ...
          const b = Number(siswa.B || siswa.bolos || 0);
    ...
    if (details.length === 0) {
      return (
        <View style={{ padding: 10 }}>
          <Text style={styles.emptyDetailText}>Tidak ada rincian log harian.</Text>
        </View>
      );
    }

    const listAlpa = details.filter(d => d.statusHarian === 'A');
    const listBolos = details.filter(d => d.statusHarian === 'B');
    const listSakit = details.filter(d => d.statusHarian === 'S');
    const listIzin = details.filter(d => d.statusHarian === 'I');
          ...
          <View style={[styles.groupCard, { borderLeftColor: '#DC2626' }]}>
            ...
            <View style={styles.groupBody}>
              {listAlpa.map((item, idx) => (
                <Text key={idx} style={styles.listItem}>• Tanggal: {item.tanggal}</Text>
              ))}
            </View>
          </View>
        )}
          ...
          <View style={[styles.groupCard, { borderLeftColor: '#92400E' }]}>
            ...
            <View style={styles.groupBody}>
              {listBolos.map((item, idx) => (
                <Text key={idx} style={styles.listItem}>• Tanggal: {item.tanggal} ({item.catatanMapel})</Text>
              ))}
            </View>
          </View>
        )}
          ...
          {!selectedKelas ? (
            ...
            <ScrollView style={styles.tableScrollView}>
              {siswaKerawanan.map((siswa, index) => {
                ...
                return (
                  <View key={siswa.idSiswa || index} style={[styles.tableRow, index % 2 === 1 && styles.tableRowEven]}>
                    <Text style={[styles.tdCell, styles.colNo]}>{index + 1}</Text>
                    <Text style={[styles.tdCell, styles.colNama]} numberOfLines={1}>{siswa.namaSiswa || siswa.nama}</Text>
                    <Text style={[styles.tdCell, styles.colKelas]} numberOfLines={1}>{siswa.namaKelas || siswa.kelas || '-'}</Text>
          ...
          <View style={styles.modalBulanContent}>
            ...
            {loadingKelas ? (
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <TouchableOpacity style={styles.bulanOption} onPress={() => { setSelectedKelas(item); setShowKelasModal(false); }}>
                    <Text style={[styles.bulanOptionText, selectedKelas?.value === item.value && styles.bulanOptionActive]}>{item.label}</Text>
                  </TouchableOpacity>
                )}
              />
            )}
...
const styles = StyleSheet.create({
  ...
  tableCard: { flex: 1, margin: 12, backgroundColor: '#FFF', borderRadius: 10, borderWidth: 1, borderColor: '#CBD5E1', overflow: 'hidden', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2 },
  tableHeaderRow: { flexDirection: 'row', backgroundColor: '#F1F5F9', paddingVertical: 10, paddingHorizontal: 4, borderBottomWidth: 2, borderBottomColor: '#CBD5E1', alignItems: 'center' },
  thCell: { fontSize: 10, fontWeight: 'bold', color: '#334155', textAlign: 'center' },
  tableBodyContainer: { flex: 1, backgroundColor: '#FFF' },
  ...
  tdCell: { fontSize: 10, color: '#334155', textAlign: 'center' },
  ...
  miniBadge: { paddingHorizontal: 3, paddingVertical: 2, borderRadius: 4, borderWidth: 1 },
  miniBadgeText: { fontSize: 8.5, fontWeight: 'bold' },
